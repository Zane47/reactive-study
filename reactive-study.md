...
提供任意数量的元素序列. 接收到订阅者Subscriber发出的订阅后会根据订阅者的需求逐次调用下面的方法来进行事件传播.

onSubscribe; onNext*n; (onError|onComplete)?可选

onSubscriber肯定被调用1次, 然后是0~n次的onNext, 最后是onError或者onComplete结束订阅流程. 订阅者主动取消当前订阅时可以不需要onError|onComplete.
...
* 如果publisher进程失败，需要调用onError告知订阅者. -> 订阅者有时间做错误处理, 资源清理
* Publisher.subscribe()内部需调用onSubscribe(Subscriber s). onSubscribe通知订阅者该订阅已开始 -> 为了让Subscriber做好初始化, 开始准备接受后续元素.
* 允许同时存在多个订阅者, 类似广播 -> Publisher和Subscriber的关系: 1对n
...
* 为了收到onNext发送的信号, 订阅者必须调用Subscriptions.request(long n)来向发布者请求数据 -> 为了让订阅者决定什么时候如何来处理上游传下来的元素

。

来告知发布者我有这个需求，这个规则的意义在于让订阅者来决定什么时候和如何去处理上游传下来的元素，从而确保他有足够的资源来进行处理，并且呢规范也推荐我们在力所能及的范围内，每次请求尽可能多的元素。这样的话相比较每次请求一个元素会更加有效率。第二呢的complete和on error方法不能够调用subscription或者是public上的任何方法，这个呢主要是为了防止在处理这两个终结信号的操作的时候，publisher和他们如果是在不同的县城上去执行的话，可能会出现一些静态条件。那第三个如果当前的subscriber有活跃的订阅的话，那么他如果在接收了另外一个订阅阿文传送的一个方法当中的时候呢，必须要去调用subscription cancel来取消另外一个订阅，这样呢就能够保证我们的订阅者不会同时与多个发布者发生交互，如果我们不这么做的话，就可能因为资源泄露而产生一些稀奇古怪的bug。此外呢，即使订阅者已经通过调用了到cancel来取消了当前的这个订阅，他还是要做好准备去接受可能出现的nex信号，这是因为当我们去调用天色方法的时候呢，发布者并没有保证他会立即执行，可能发布者在真正的去处理这个取消订阅的操作之前依然会向下游去发送一些元素。那这个时候就需要去处理这样的一些元素。那第三个接口呢是subscription的接口也比较直接，他一共只经历了两个方法，第一个方法就是去请求。是另外一个cancel的，就是取取消这个订阅，那它是属于表述当前发布者和订阅者之间的订阅关系的一个对象，他会在调用订阅者定义的IP的方法的时候传递给订阅者，那这样呢订阅者就获得了当前和当前的这个活跃订阅的这样的一个关联关系，我们来看一下subscription这个接口的时候，他的一些他的一些处理的规则。首先request和cancel都必须在当前subscribe的上下文当中被调，这样的确保发布者和订阅者这个关系的唯一性。另外的一个request必须支持请求最多的二的63次方减一个元素，也就是JAVA中long型的长整型的max value这么多元素啊，基本上你就可以把它看作是无限多个元素，因为呢我们换算一下就像我们每一纳秒发送一个元素的话，如果要发送这么多元素的话需要292年，也就是说你目前计算机的硬件条件来看，你可以把它理解为最多请求的这样一个value的话，就是一个无限元素的序列。那最后一个接口呢就是接口它代表的是一个处理的状态，所以它既是一个怕也是一个这个接口，大家知道他的存在就可以了，为什么呢？因为我们今天会主要介绍的reactor框架里面，在3.5版本之后就已经作废了。其中的processor的一些实践类，比如说processor和flux转而使用。
